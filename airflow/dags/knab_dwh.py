...
with DAG(
    ...
    #Fact Job Loading
    task_id = 'fac_fin_transaction_load'
    fac_psql_command='psql -h localhost -p '+str(pg_port)+' -U postgres -v ON_ERROR_STOP=1 -d postgres -f ' + repo_home_dir +'/etl_repo/fact/'
    command = fac_psql_command+task_id+'.sql >'+logfile_command+task_id+'.log'
    run_fac_fin_transaction = BashOperator(
        task_id=task_id,
        bash_command=command,
    )

    task_id = 'fac_acc_balance_load'
    command = fac_psql_command + task_id + '.sql >' + logfile_command + task_id + '.log'
    run_fac_acc_balance = BashOperator(
        task_id=task_id,
        bash_command=command,
    )

    task_id = 'fac_dwh_eod_position'
    fac_psql_command = 'psql -h localhost -p ' + str(pg_port) + " -U postgres -v ON_ERROR_STOP=1 -v p_bsdate="+'"'+"'"+business_date+"'"+'"'+" -d postgres -f " + repo_home_dir + '/etl_repo/fact/'
    command = fac_psql_command + task_id + '.sql >' + logfile_command + task_id + '.log'
    run_fac_dwh_eod_position = BashOperator(
        task_id=task_id,
        bash_command=command,
    )
    ...
    task_id = 'DNB_DGS_Daily_Report'
    command = 'cd ' + repo_home_dir + '/etl_repo/report;' + \
              'python3 -c ' + task_id+".py" + ' '+business_date
    run_dnb_dgs_daily_report = BashOperator(
        task_id=task_id,
        bash_command=command,
    )
    ...
    run_staging_done >> [run_dim_calendar_date, run_dim_calendar_time, run_dim_customer, run_dim_account] >> run_dimension_done
    run_dimension_done >> [run_fac_fin_transaction, run_fac_acc_balance]
    run_fac_fin_transaction >> run_fac_dwh_eod_position >> run_batch_end
    run_fac_acc_balance >> run_batch_end >> run_dnb_dgs_daily_report
